    ...
    try {
      await base44.entities.PartnerRequest.create({
        ...
        notes: [
          ...
          referralCode ? `[Indicação] ref=${referralCode}` : '',
        ].filter(Boolean).join('\n'),
        status: 'pendente',
      });
      setStep('countdown');
    ...
    return (
      ...
      style={{ background: 'linear-gradient(160deg, #0d3320, #145a32)' }}>
        <motion.div
          ...
          <h2 className="text-xl font-black text-foreground mb-3">Seja um Parceiro Comercial!</h2>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background">
         <motion.div
           ...
           <Button className="w-full h-12 font-bold text-base bg-green-600 hover:bg-green-700 shadow-lg" onClick={() => navigate('/')}>
             Voltar ao Início
           </Button>
         </motion.div>
       </div>);
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <div className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b px-4 py-3 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-black text-base">Cadastro de Parceiro Comercial</h1>
          <p className="text-xs text-muted-foreground">Preencha os dados da sua empresa</p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 space-y-5">
        ...
        <div className="pt-2 pb-8">
          {!valid &&
          <p className="text-xs text-destructive text-center mb-3">Preencha todos os campos (Obrigatório) para continuar.</p>
          }
          <Button
