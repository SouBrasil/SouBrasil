  ...
  const handleApprove = async (r) => {
    ...
    try {
      ...
      const created = await base44.entities.Partner.create({
        ...
        phone: r.phone || r.whatsapp || '', image_url: r.logo_url || r.business_photo_url || '',
      ...
      if (r.owner_email) {
        await base44.entities.PartnerAccess.create({ partner_id: created.id, partner_name: r.business_name, email: r.owner_email, password_hash: defaultPassword, must_change_password: true, active: true });
        await base44.integrations.Core.SendEmail({ to: r.owner_email, subject: '🎉 Seu cadastro foi aprovado — Portal Parceiro Sou Brasil', body: `Olá, ${r.owner_name || r.business_name}!\n\nSua solicitação foi APROVADA! 🎊\n\n📧 E-mail: ${r.owner_email}\n🔑 Senha provisória: ${defaultPassword}\n\nAcesse: ${window.location.origin}/PartnerPortal\n\n— Equipe Sou Brasil` });
      }
      await base44.entities.PartnerRequest.update(r.id, { status: 'aprovado', notes: notes[r.id] || '' });
      qc.invalidateQueries(['ap-requests-list']);
      toast.success('Aprovado! Parceiro criado e e-mail enviado.');
    } catch (err) {
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge className={`text-[10px] ${r.status === 'aprovado' ? 'bg-green-100 text-green-700' : r.status === 'recusado' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>
                          {r.status === 'aprovado' ? 'Aprovado' : r.status === 'recusado' ? 'Recusado' : 'Pendente'}
                        </Badge>
                        {r.status === 'pendente' && (
                  ...
                  {isOpen && (
                    <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        {r.address && <div className="col-span-2 bg-slate-50 rounded-lg p-2"><p className="text-[10px] text-slate-400">Endereço Completo</p><p className="font-medium">{r.address}</p></div>}
                        {r.benefit_description && <div className="col-span-2 bg-green-50 rounded-lg p-2"><p className="text-[10px] text-slate-400">Benefício oferecido</p><p className="font-medium text-green-800">{r.benefit_description}</p></div>}
                        ...
                        <div className="bg-slate-50 rounded-lg p-2"><p className="text-[10px] text-slate-400">Data da solicitação</p><p className="font-medium">{new Date(r.created_date).toLocaleDateString('pt-BR')}</p></div>
                      </div>

                      {/* Photos */}
                      {(r.logo_url || r.business_photo_url) && (
                        <div>
